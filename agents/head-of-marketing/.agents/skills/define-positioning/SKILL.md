...
The Head of Marketing OWNS `product-marketing-context.md`. No other
...
2. **Read the existing doc if present.** If
   `product-marketing-context.md` exists, read it so this run is an
...
6. **Write atomically.** Write to
   `product-marketing-context.md.tmp`, then rename to
   `product-marketing-context.md`. Single file at agent root. NOT
...
7. **Append to `outputs.json`.** Read existing array, append a new
   ...
   ```json
   {
     ...
     "path": "product-marketing-context.md",
     "status": "draft",
     "createdAt": "<ISO-8601>",
     "updatedAt": "<ISO-8601>"
   }
...
- `product-marketing-context.md` (at the agent root — live document)
